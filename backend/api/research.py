...
async def start_research(request: ResearchRequest, db: AsyncSession = Depends(get_db)):
    ...
    asyncio.create_task(_run_research_task(session_id, request.topic))
...
async def _run_research_task(session_id: str, topic: str):
    ...
    try:
        final_state = await run_pipeline(topic, on_agent_update=on_agent_update)
