...
def search_node(state: ResearchState) -> ResearchState:
    try:
        results = run_search_agent(state["topic"])
...
def research_node(state: ResearchState) -> ResearchState:
    try:
        summaries = run_research_agent(state["topic"], state["search_results"])
...
def factcheck_node(state: ResearchState) -> ResearchState:
    try:
        verified = run_factcheck_agent(state["topic"], state["summaries"])
...
def report_node(state: ResearchState) -> ResearchState:
    try:
        result = run_report_agent(state["topic"], state["verified_facts"], state["summaries"])
        return {
            **state,
